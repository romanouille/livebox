...
{ 
  ...
  do
	case $i in
		...
		;;
    	*)
		echo "  3 Vendor Configuration File.sh Usage: "
		echo "   --path=[PCBPATH]"
		echo "   --url=[URL]"
		echo "   --username=[USER]"
		echo "   --password=[PASS]"
		echo "   --capath=[PATH]"
		echo ""
		echo "  e.g. 3 Vendor Configuration File.sh --path=ManagemenServer.QueuedTransfers.Entry.1 --url=http://my.acs.com/download.cgi?"
  ...
  if [ ! -n "$URL" ]; then
    finish_transfer 9002 "Not a valid url"
    exit 1
  fi
...
{ 
  ...
  RESPONSE=$(eval /usr/bin/curl --write-out %{http_code} --silent --fail  --anyauth $1 --output /dev/null $AUTHSTRING $SSL $URL)
  CURLERROR=$?
